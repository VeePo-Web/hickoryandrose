...
/**
 * SectionIndicator — Floating editorial sidebar showing current section name.
 * Desktop-only (hidden on touch devices and <lg screens).
 * Reads `aria-label` from `<section>` elements inside `#main-content`.
 */

const SectionIndicator = memo(() => {
  ...
  useEffect(() => {
    ...
    // Initial gather after paint
    const timer = setTimeout(gather, 500);
    window.addEventListener("scroll", updateSection, { passive: true });

    // Re-gather on lazy-load layout shifts
      ...
      {visible && (
        ...
        >
          ...
          <div className="relative w-px h-24">
            ...
            />
            {/* Gold glow dot at progress head */}
            <motion.div
